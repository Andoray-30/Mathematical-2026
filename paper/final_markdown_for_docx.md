...
取$\eta = 0.6$，得组合权重：$w = (0.1514, 0.4955, 0.3531)$。AHP主观权重与熵权客观权重的对比如图6所示。

[插入图6：AHP与熵权权重对比，figures/final/weight_comparison.png]
...
5.jpg综合质量最高，得益于其在三个维度上的均衡表现（$S_{tech}=81.54$，$S_{str}=93.75$）；4.png质量最低，主要因为技术质量极低（$S_{tech}=14.63$）。各图像三维度得分分布如图2所示，TOPSIS贴近度排序如图3所示。

[插入图2：各图像三维度得分分布，figures/final/image_dimension_scores.png]

[插入图3：TOPSIS贴近度排序，figures/final/image_ranking.png]

### 7.3 稳健性验证

**灰色关联校验**：以各维度最大值为理想序列，计算灰色关联度，得到GRA排序。TOPSIS与GRA排序的Spearman相关系数为0.976（p<0.001），Kendall相关系数为0.929（p<0.001），Top-3一致率为100%。两种方法的排序一致性如图4所示。

[插入图4：TOPSIS与灰色关联排序一致性，figures/final/gray_topsis_consistency.png]

**敏感性分析**：进行融合系数变化（$\eta=0.5, 0.6, 0.7$）、AHP矩阵扰动、去掉语义维度、等权方案等6种场景实验。结果表明，所有场景下Top-3排名保持稳定（5.jpg、7.jpg、6.jpg），8张图像的平均排名标准差仅为0.09，最大排名波动为1。敏感性分析结果如图5所示。

[插入图5：敏感性分析龙卷风图，figures/final/sensitivity_tornado.png]
...
[插入图7：时序失稳分数曲线，figures/final/video_instability_curve.png]
...
分析表明，当前最优先的参数调整为：提高分辨率（优先级0.1126）、增加采样步数（优先级0.0943）、增强局部重绘（优先级0.0819）。这反映了当前样本在技术质量维度上存在明显短板。各参数对各样本的优先级热力图如图8所示。

[插入图8：参数优化优先级热力图，figures/final/parameter_priority_heatmap.png]
