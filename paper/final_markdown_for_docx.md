...
三个问题构成"评价—诊断—优化"的递进链条。问题一建立数学框架，为后续评价提供理论基础；问题二将框架应用于具体样本，通过组合赋权和排序实现综合评价，并验证结果可靠性；问题三将评价从静态图像扩展到动态视频，引入时序失稳检测。最后，参数优化建议形成闭环，从质量短板反推生成参数调整方向。总体技术路线如图1所示。

[插入图1：总体框架图，figures/final/framework_flowchart.png]
...
取$\eta = 0.6$，得组合权重：$w = (0.1514, 0.4955, 0.3531)$。AHP主观权重与熵权客观权重的对比如图2所示。

[插入图2：AHP与熵权权重对比，figures/final/weight_comparison.png]
...
5.jpg综合质量最高，得益于其在三个维度上的均衡表现（$S_{tech}=81.54$，$S_{str}=93.75$）；4.png质量最低，主要因为技术质量极低（$S_{tech}=14.63$）。各图像三维度得分分布如图3所示，TOPSIS贴近度排序如图4所示。

[插入图3：各图像三维度得分分布，figures/final/image_dimension_scores.png]

[插入图4：TOPSIS贴近度排序，figures/final/image_ranking.png]

### 7.3 稳健性验证

**灰色关联校验**：以各维度最大值为理想序列，计算灰色关联度，得到GRA排序。TOPSIS与GRA排序的Spearman相关系数为0.976（p<0.001），Kendall相关系数为0.929（p<0.001），Top-3一致率为100%。两种方法的排序一致性如图5所示。

[插入图5：TOPSIS与灰色关联排序一致性，figures/final/gray_topsis_consistency.png]

**敏感性分析**：进行融合系数变化（$\eta=0.5, 0.6, 0.7$）、AHP矩阵扰动、去掉语义维度、等权方案等6种场景实验。结果表明，所有场景下Top-3排名保持稳定（5.jpg、7.jpg、6.jpg），8张图像的平均排名标准差仅为0.09，最大排名波动为1。详细敏感性分析结果见附录D。
...
8张测试图像覆盖写实风景、人物肖像、艺术插画、像素艺术等内容类型，不同内容类型对各指标的敏感程度存在显著差异。写实类图像（如5.jpg、7.jpg）在结构完整性维度得分较高，因为其边缘清晰、几何关系明确；而艺术插画类图像（如6.jpg）的技术质量指标（如Laplacian方差）天然偏低，但语义保真度表现稳定。

通过逐样本短板诊断可以发现：技术质量维度的区分度最大（熵权0.4888），其受内容类型影响最为明显。像素艺术类图像（4.png）的分辨率和清晰度指标远低于写实图像，但这并不意味着其"质量差", 而是指标本身对该内容类型的适用性有限。结构完整性维度（熵权0.5078）同样存在内容类型偏差：人物肖像类图像的手部和面部缺陷检测更为敏感，而风景类图像的结构完整性主要由全局代理指标决定。

综合来看，当前指标体系对写实类内容最为友好，对艺术风格类内容存在一定程度的技术指标"误伤"。在实际应用中，建议根据内容类型调整指标权重或引入风格自适应变换函数。
...
基于上述指标体系和异常检测框架，本文给出AIGC视频出现时序失稳的必要条件：

**条件1（单帧异常）**：存在至少一个时序指标的稳健z-score超过阈值，即$\exists k, z_t^{(k)} > \tau_k$，其中$\tau_k$为指标$k$的异常阈值。

**条件2（多指标协同）**：时序失稳分数$I_t$超过统计控制限，即$I_t > \text{med}(I) + \lambda \cdot \text{MAD}(I)$，本文取$\lambda = 2.5$。

**条件3（非运动性）**：异常帧的Warp-SSIM损失显著高于普通SSIM损失，即光流补偿后残余差异仍然显著，表明异常不能完全由自然运动解释。

三个条件同时满足时，可判定该帧为候选时序失稳帧。仅满足条件1和条件2而条件3不满足的帧，可能由快速运动、遮挡变化等自然因素导致，属于正常波动而非生成失稳。本文分析的车流视频中，15个异常帧均满足条件1和条件2，但条件3不完全满足，因此判定为自然运动导致的正常波动。
...
[插入图6：时序失稳分数曲线，figures/final/video_instability_curve.png]
...
分析表明，当前最优先的参数调整为：提高分辨率（优先级0.1126）、增加采样步数（优先级0.0943）、增强局部重绘（优先级0.0819）。这反映了当前样本在技术质量维度上存在明显短板。各参数对各样本的优先级热力图如图7所示。

[插入图7：参数优化优先级热力图，figures/final/parameter_priority_heatmap.png]
...
| 采样步数 | 0.0943 | 增加 |
| 局部重绘 | 0.0819 | 增强 |
| CFG Scale | 0.0654 | 适当调低 |
| 去噪强度 | 0.0521 | 微调 |
| 提示词权重 | 0.0387 | 优化结构 |
| 随机种子 | 0.0215 | 多次采样筛选 |

表2给出了各参数的优先级排序和调整方向。分辨率和采样步数的优先级最高，这与当前样本技术质量维度短板最为突出的结论一致。建议优先提升生成分辨率，其次增加采样步数以改善细节质量。
...
[插入附录图：敏感性分析龙卷风图，figures/final/sensitivity_tornado.png]
